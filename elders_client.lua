...
local function check_override_ui(players, player_id)
   ...
   if client_player then
      if client_player.kingdom == "elders:kingdoms:elders" then
         -- hot load elders ui mod
         _radiant.res.apply_manifest("/elders/ui/manifest.json")
      end
   end
end
...
local function trace_player_service()
   _radiant.call('stonehearth:get_service', 'player')
      :done(function(r)
         ...
         player_service_trace = player_service:trace('elders ui change')
...
radiant.events.listen(elders, 'radiant:init', function()
